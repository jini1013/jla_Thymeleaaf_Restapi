...
//Then
        assertNotNull("Not null 발생", exception);
...
//Given
        ...
        int orderCount = 2;
        Long orderId = orderService.order(member.getId(), item.getId(),orderCount);
        assertEquals("주문취소전 재고수량", 8 , item.getStockQuantity());
//When
...
//Then
    ...
    public void 주문검색() throws Exception {
        Member member = createMember("몽타", new Address("서울", "성내로", "180"));
        Item item = createBook("부트", 10000, 10); //이름, 가격, 재고
        int orderCount = 3;
        Long orderId = orderService.order(member.getId(), item.getId(),orderCount);
